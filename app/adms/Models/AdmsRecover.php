...
/**
 * Solicita novo link para alterar senha
 */
class AdmsRecover 
{
    ...
    */
    ...
    private function valConfEmail(): void
    {
            ...
            if($upNewConfEmail->getResult()){
                ...
                $_SESSION['msg'] = "<p style='color: #f00;'>Erro: Recuperação de senha não enviado, tente novamente!</p>";
                $this->result = false;
            }
    }
    ...
    private function sendEmail():void
    {
        ...
        if($sendEmail->getResult()){
            $_SESSION['msg'] = "<p style='color: green;'>Novo link enviado com sucesso. Acesse a sua caixa de e-mail para recuperar a senha!</p>";
            $this->result = true;
        }else{
    ...
    private function emailHTML(): void
    {
        ...
        $this->emailData['contentHtml'] .= "Para continuarmos o processo de recuperação de sua senha, clique no link abaixo ou cole o endereço no seu navegador:<br><br>";
        $this->emailData['contentHtml'] .= "<a href='{$this->url}'>{$this->url}</a><br><br>";
        $this->emailData['contentHtml'] .= "Se você não solicitou essa alteração, nenhuma ação é necessária. Sua senha permanecerá a mesma até que voce ative este código.<br><br>"; 
        $this->emailData['contentHtml'] .= "Caso não tenha solicitado essa alteração, basta ignorar esse email e elimina-lo para ningué o utilizar.<br><br>"; 
    }
    ...
    private function emailText(): void
    {
        ...
        $this->emailData['contentText'] .= "Você solicitou alteração de senha.!<br><br>\n\n";
        $this->emailData['contentText'] .= "Para continuarmos o processo de recuperação de sua senha, clique no link abaixo ou cole o endereço no seu navegador: \n\n";
        $this->emailData['contentText'] .= $this->url . "\n\n";
        $this->emailData['contentText'] .= "Se você não solicitou essa alteração, nenhuma ação é necessária. Sua senha permanecerá a mesma até que voce ative este código.\n\n";  
        $this->emailData['contentText'] .= "Caso não tenha solicitado essa alteração, basta ignorar esse email e elimina-lo para ningué o utilizar.\n\n"; 
    }
}