 ...
 */
class AdmsValidateAddUserSingle
{
    ...
    public function validateUserSingle(string $user, bool|null $edit = null, int|null $id = null): void
    {
       ...
       if(($this->edit == true) and (!empty($this->id))){
            $verifyUserSingle->fullRead("SELECT id FROM adms_users WHERE user =:user id <>:id LIMIT :limit", "user={$this->email}&id={$this->id}&limit=1");
       }else{
       ...
       if(!$this->resultBd){
        ...
        $_SESSION['msg'] = "<p style='color:#f00';>Erro: Usuário já utilizado, por favor verifique!</p>";
        $this->result = false;
       }
    }
}
